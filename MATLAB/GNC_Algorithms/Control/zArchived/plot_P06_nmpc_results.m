% filename: MATLAB/GNC_Algorithms/Control/NMPC/plot_P06_nmpc_results.m
%
% Plot results for Problem 6 NMPC point stabalization (PX4 in the loop).
% Requires a MAT-file with 'log_data' saved by save_log_data().

function plot_P06_nmpc_results(matfile)
    S = load(matfile);
    L = S.log_data;
    ...
    %% 1) 3D trajectory: actual vs predicted
    figure;
    hold on; grid on; axis equal;

    % 3D position: x, y, z
    plot3(X(1,:),  X(2,:),  -X(3,:),  'b',  'LineWidth', 1.5);   % actual
    plot3(Xp(1,:), Xp(2,:), -Xp(3,:), 'g:', 'LineWidth', 1.5);   % prediction

    xlabel('x [m]');
    ylabel('y [m]');
    zlabel('z [m]');          % <- important for 3D plot
    legend('actual','predicted','Location','Best');
    title('3D Trajectory: actual vs NMPC predicted');
    view(3);
