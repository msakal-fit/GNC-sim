% filename: MATLAB/GNC_Algorithms/Control/run_P03_LQT_Obs.m

function run_P03_LQT_Obs()
    ...
    % load trajectory generated by offline NMPC
    S = load('ref_traj_P08b_obs_for_LQT.mat');

    ref_traj = S.ref_traj;
    t_ref    = ref_traj.t;      % N_ref x 1
    X_ref    = ref_traj.X;      % 13 x N_ref
    U_refN   = ref_traj.U;      % 4 x N_ref (optional feedforward)
    Ts_ref   = ref_traj.Ts;     % sampling time used in offline NMPC
    N_ref    = numel(t_ref);
    ...
    log_data = initialize_logging();
     ...
     % simulation loop
    fprintf('Starting the P03 - LQT+Obstacle simulation...\n');
    t = 0;
    ...
    while t < flight_duration
        ...
        idx_ref = round(t / Ts_ref) + 1;
        idx_ref = max(1, min(N_ref, idx_ref));
    ...
    save_log_data(log_data, 'log_p03_lqt.mat');
    %plot_P02_lqt_results('log_p02_lqt.mat');
    plot_tracking_results('log_p03_lqt.mat', 'P03 - LQT+Obstacle');
